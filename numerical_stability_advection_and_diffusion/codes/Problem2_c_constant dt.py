...
alpha = 100.0  # diffusion coefficients
...
print N
order1 = [(1/x) ** 1 for x in [float(i) for i in N]]
order2 = [(1/x) ** 2 for x in [float(i) for i in N]]
order3 = [(1/x) ** 3 for x in [float(i) for i in N]]
order4 = [(1/x) ** 4 for x in [float(i) for i in N]]

plt.loglog(N, RMS1, label = "1st-order-upwind")
plt.loglog(N, RMS2, label = "2nd-order-central")
plt.loglog(N, RMS3, label = "2nd-order-upwind")

plt.loglog(N, order1, '--', label = '1st order reference')
plt.loglog(N, order2, '--', label = '2nd order reference')
plt.loglog(N, order3, '--', label = '3th order reference')
plt.loglog(N, order4, '--', label = '4th order reference')
plt.xlabel('The number of points in the domain(Nx)')
plt.ylabel('RMS')
plt.title('RMS of different spatial scheme with Crank-Nicolson time scheme')
plt.legend(loc = 'lower right')
plt.savefig('Problem2_c_constant dt.png')
plt.show()
