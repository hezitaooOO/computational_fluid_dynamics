...
def gs_sor(A, b, w=1.4, tol=1.0e-5):
    rk_list = []
    index_list = []
    n = len(b)
...
for i in range(len(Re_list)):
    ...
    phi, iteration, rk = gs_sor(Derivative - 1. / Re * DivGrad, get_q(xc, yc, Re, n), w)
...
#
plt.plot(iteration_list[0], rk_list[0], label = 'Re = 0.01')
plt.plot(iteration_list[1], rk_list[1], label = 'Re = 1')
plt.plot(iteration_list[2], rk_list[2], label = 'Re = 100')
plt.plot(iteration_list[3], rk_list[3], label = 'Re = 500')


plt.title('Instantenous residual rk, versus the iteration number k for different Reynolds number Re')
...
plt.legend(loc = 'best')
plt.savefig('Instantenous residual rk, versus the iteration number k for different Reynolds number Re')
print rk_list[0]